...
plot(lmm1)
qqnorm(resid(lmm1)) #Wasn't working
summary(lmm1) #Lower in Winter and Autumn than in summer. Decreasing over time

lmm2 <- lmer(NO3 ~ Season + Year + (1|SITE_ID), data=Air_Data_NC)
AIC(lmm2)
plot(lmm2)
qqnorm(resid(lmm2)) #Wasn't working
summary(lmm2)

lmm3 <- lmer(HNO3 ~ Season + Year + (1|SITE_ID), data=Air_Data_NC)
AIC(lmm3)
plot(lmm3)
qqnorm(resid(lmm3)) #Wasn't working
...
AIC(lmm4)
plot(lmm4)
qqnorm(resid(lmm4)) #Wasn't working
summary(lmm4)

#Can say which gases have seasonal effects
#HNO3 and NO3 have very negative AIC values while SO4 has 69.91.
#HNO3 and SO4 have significant p values, indicating that they display seasonality. 
